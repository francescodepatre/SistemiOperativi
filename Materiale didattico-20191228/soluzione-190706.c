...
- il server Ps offre un servizio concorrente (un figlio per ogni connessione) alla porta 876 oppure alla porta 8765 (giustificare)
...
int main()
{
  ...
  if((pidStore=fork())<0)  //creo il figlio Pstore, che sara' permanente fino alla fine del programma
    {
      perror("fork Pstore");
      exit(-1);
    }
