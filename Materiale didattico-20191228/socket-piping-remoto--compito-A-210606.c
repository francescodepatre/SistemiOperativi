...
Il server riceve dal client richieste per il piping
di due comandi, ovvero "cmd1|cmd2"
L'output del cmd2 deve essere ridiretto nella socket.
L'eventuale notifica al server di un segnale SIGUSR1
deve far sì che il server effettui il log dell'indirizzo dei client che si connettono.
...
#define DEFAULTPORT 9876	/* E' porta non privilegiata , mentre 987 e' privilegiata e utilizzabile solo da processi di root */
