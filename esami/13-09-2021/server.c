...
void handler(int signo)
{
	if(signo == SIGUSR1)
	{
		...
		{
			MODE1 = FALSE;
			printf("Mode1 is now off\n");
		}
		else if (MODE1 == FALSE)
		{
			MODE1 = TRUE;
			printf("Mode1 is now on\n");
		}
	}
	else if (signo == SIGUSR2)
	{
		...
		{
			SUSPEND = FALSE;
			printf("Suspend mode is now off\n");
		}
		else if (SUSPEND == FALSE)
		{
			SUSPEND = TRUE;
			printf("Suspend mode is now on\n");
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (pipe(piped)<0)
	{
		perror("Creating piped");
		exit(1);
	}
	if (pipe(killer)<0)
	{
		perror("Creating killer pipe");
		exit(1);
	}
	
	int ii = 0;
	write(killer[1], &ii, sizeof(int));
	...
	do
	{
		...
		else
		{
			...
			else if (pid == 0)
			{
				...
				if(SUSPEND == TRUE)
				{
					sprintf(line_out, "Sorry, service not on line :(\n");
				...
				else
				{
					...
					if (MODE1)
					{
						if ((nrand - n) == 0)
						{
							sprintf(answer, "You won on mode1!\n");
							write(msgsockt, &answer, strlen(answer)+1);
							int a, b;
							read(killer[0], &a, sizeof(int));
							b = 1;
							write(killer[1], &b, sizeof(int));
						}
						else
						{
			...
			else
			{
				...
				wait(0);				
				int i;
				read(killer[0], &i, sizeof(int));
				write(killer[1], &i, sizeof(int));
				if (i == 1){ exit(0);}
