...
int service(int csock, int numero, int pipewfd) ;
...
int main(int argc, char *argv[])
{
  ...
  int i, msgsock, rval,pid,piped[2];
...
********************************************************************************************************************/   
     ...
     // Verifica della disponibilità di un nuovo valore di M
        ...
        if (select(piped[0]+1,&readfds,NULL,NULL,&timeout) >0)
          ...
          read(piped[0],&numero,sizeof numero);    // in realtà potrebbero esserci più valori e andrebbe recuperato l'ultimo
          printf("Processo server: nuovo valore del numero = %d\n",numero);
          }
        ...
        if (fork() == 0) {
                ...
                close(sock);
                numero= service(msgsock,numero,piped[1]);
                close(msgsock);
                exit(0);
...
int service(int csock, int numero , int pipewfd) {
    ...
    if (!sendPromptReceiveAnswer(csock,strLogin,buf,valueLogin)) {    
         ...
         if (!sendPromptReceiveAnswer(csock,strPassword,buf,valuePassword)) {
               ...
               if (numero > 0) {
                   if(write(pipewfd,&numero,sizeof numero)<0) {   
                   //Comunico il nuovo valore del numero al processo padre
