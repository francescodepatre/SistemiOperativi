...
printf("Server attivo sulla porta #%d\n",ntohs(server.sin_port));
  ...
  do{
    ...
    /* Attesa di una connessione */	 
      ...
      if((rval= select(sock+1,&readfds,NULL,NULL,&tmv))<0)
      {
	  perror("select: ");
	  exit(-1);
      }
      ...
      if(rval==0)
      {
	  printf("Nessun cliente si e' connesso negli ultimi %d secondi\nTermino\n",SELECT_TIMEOUT);
	  exit(0);
      }

      /* Non ci sono dubbi su quale descrittore possa essere pronto...  */


      msgsock= accept(sock,(struct sockaddr *)&client,(unsigned int *)&length);
